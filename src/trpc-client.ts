...
export class TRPCClient {
		...
		this.httpClient.interceptors.response.use(
			...
			async (error) => {
				...
				if (error.response?.status === 401) {
					console.error('[BLOCKED] Authentication failed - NextAuth session may be expired');
					throw new Error('Authentication expired. Please re-authenticate with NextAuth cookies.');
				} else if (error.response?.status === 403) {
					console.error('[BLOCKED] Access forbidden - insufficient permissions');
					throw new Error('Access forbidden. Check your permissions or re-authenticate.');
	...
	async call<T = any>(method: string, params?: any): Promise<T> {
		...
		try {
			...
			console.log(`[COOKIE] Request cookies: ${cookieHeader ? 'present' : 'missing'}`);
			...
			try {
				...
				console.log(`[SUCCESS] tRPC ${method} GET response status: ${getResponse.status}`);

				if (getResponse.data.error) {
					console.error(`[ERROR] tRPC ${method} GET returned error:`, getResponse.data.error);
					throw new Error(
				...
				const postResponse = await this.httpClient.post<TRPCResponse<T>>(postUrl, request, {
					headers: {
						'Content-Type': 'application/json',
					},
				});

				console.log(`[SUCCESS] tRPC ${method} POST response status: ${postResponse.status}`);

				if (postResponse.data.error) {
					console.error(`[ERROR] tRPC ${method} POST returned error:`, postResponse.data.error);
					throw new Error(
			...
			if (error && typeof error === 'object' && 'response' in error) {
				...
				if (axiosError.response?.status === 401) {
					console.error('[BLOCKED] tRPC 401 Unauthorized - session may be expired or invalid');
				} else if (axiosError.response?.status === 403) {
					console.error('[BLOCKED] tRPC 403 Forbidden - insufficient permissions');
				} else if (axiosError.response?.status === 404) {
					console.error('[BLOCKED] tRPC 404 Not Found - endpoint may not exist');
	...
	async getNewsWithPagination(params?: any): Promise<any[]> {
		return this.call<any[]>('news.findWithPagination', params || { take: 20, skip: 0 });
	}
