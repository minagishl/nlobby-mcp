...
export class NLobbyMCPServer {
			...
			return {
				tools: [
...

					{
						...
						description: 'Open browser for manual login to N Lobby (no credentials required)',
						inputSchema: {
							type: 'object',
							properties: {},
						},
				...
				switch (name) {
					...
					case 'get_calendar_events':
						try {
							...
							const calendarType =
								calendar_type === 'school' ? CalendarType.SCHOOL : CalendarType.PERSONAL;
							...
							const schedule = await this.api.getSchedule(calendarType, dateRange);
					...
					case 'test_calendar_endpoints':
						try {
							...
							const testResults = await this.api.testCalendarEndpoints(dateRange);
							...
							const reportLines = [
								...
								testResults.personal.error ? `   Error: ${testResults.personal.error}` : '',
								'',
								'[SCHOOL] School Calendar:',
								`   Status: ${testResults.school.success ? '[SUCCESS] Success' : '[ERROR] Failed'}`,
								`   Events: ${testResults.school.count}`,
								testResults.school.error ? `   Error: ${testResults.school.error}` : '',
								'',
								'[STATUS] Summary:',
								`   Total Endpoints: 2`,
								`   Successful: ${
									(testResults.personal.success ? 1 : 0) + (testResults.school.success ? 1 : 0)
								}`,
								`   Failed: ${
									(testResults.personal.success ? 0 : 1) + (testResults.school.success ? 0 : 1)
								}`,
					...
					case 'debug_connection':
						...
						const debugResult = await this.api.debugConnection(endpoint || '/news');
						return {
							content: [
								{
									type: 'text',
					...
					case 'interactive_login':
						try {
							...
							const extractedCookies = await this.browserAuth.interactiveLogin();
					...
					case 'login_help':
						...
						if (email) {
							const emailValidation = this.credentialManager.validateEmail(email);
							...
							helpMessage += this.credentialManager.getLoginGuidance(emailValidation.userType);
						} else {
	...
	private getAuthenticationRecommendations(): string {
		...
		if (
			...
			recommendations.push('1. Run interactive_login to authenticate with N Lobby');
			recommendations.push('2. Make sure to complete the login process in the browser window');
			recommendations.push('3. Wait for the "Login successful" message before proceeding');
		}
		...
		// Check if authentication is partial
			...
			recommendations.push('2. Try running interactive_login again to refresh all cookies');
			recommendations.push('3. Check if any network issues are preventing proper cookie setting');
		}
		...
		// Check if authentication is complete but endpoints are failing
			...
			recommendations.push('2. If endpoints are still failing, the issue may be server-side');
